...
def saada_kiri(kellele: str, teema: str, sisu: str, faili_tee=None):
    """Send email with optional attachment"""
    msg = EmailMessage()
    msg['From'] = "tahmazovhussejn@gmail.com"
    msg['To'] = kellele
    msg['Subject'] = teema
    ...
    if faili_tee:
        try:
            with open(faili_tee, 'rb') as f:
                faili_sisu = f.read()
                faili_nimi = faili_tee.split("/")[-1]
                msg.add_attachment(
                    faili_sisu,
            ...
            print(f"Manuse lisamine ebaõnnestus: {e}")
    
    try:
        with smtplib.SMTP('smtp.gmail.com', 587) as server:
            server.starttls(context=ssl.create_default_context())
            server.login("tahmazovhussejn@gmail.com", input("Sisesta emaili salasõna: "))
            server.send_message(msg)
            print("Kiri saadetud!")
    except Exception as e:
        print(f"Viga kirja saatmisel: {e}")
...
def registreerimine():
    ...
    while True:
        ...
        if tehe == 1:
            while True:
                password = input("Sisesta parool (vähemalt 1 täht, 1 number ja 1 erimärk): ")
                if (not re.search(r'[A-Za-z]', password) or 
                    not re.search(r'[0-9]', password)): # https://www.w3schools.com/python/python_regex.asp
                    print("Parool peab sisaldama tähti, numbreid ja erimärke!")
                    continue
                break
        elif tehe == 2:
        ...
        <html>
            <body>
                ...
                <p>Sinu konto on loodud.</p>
                <p>Kasutajanimi: {username}</p>
                <p>Parool: {password}</p>
            </body>
        </html>
        """
        saada_kiri("kasutaja@example.com", "Konto loomine", html)
        print("Registreerimine õnnestus!")
        break
...
def autoriseerimine():
    ...
    for user, pwd in users:
        if user == username and pwd == password:
            ...
            html = f"<p>Kasutaja {username} logis sisse</p>"
            saada_kiri("tahmazovhussejn@gmail.com", "Sisselogimine", html)
            print("Sisselogimine õnnestus!")
            return True
