...
def load_questions():
    """
    Küsimuste laadimine küsimuste failist
    """
    questions = {}
    try:
...
def add_question():
    """
    Lisage uus küsimus küsimuste faili
    """
    print("\n--- Lisa uus küsimus ---")
    question = input("Sisestage uus küsimus: ").strip()
    answer = input("Sisestage õige vastus: ").strip()
    
    with open("kusimused_vastused.txt", 'a', encoding='utf-8') as file:
        file.write(f"{question}:{answer}\n")
    print("Küsimus lisatud edukalt!")

def user_email(inputemail):
    """
    Looge osaleja nimest e-posti aadress
    """
    inputemail=input("Sisesta sinu mail")
    return inputemail

def get_participant_score(participant):
    """
    Abifunktsioon skoori saamiseks sorteerimiseks
    """
    return int(participant[1])

def run_quiz():
    """
    Käivita viktoriin ja salvesta tulemused
    """
    ...
    participant_email = user_email(participant_name)
