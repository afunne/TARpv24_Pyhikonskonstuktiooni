...
def init_db():
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("""CREATE TABLE IF NOT EXISTS languages (id INTEGER PRIMARY KEY, name TEXT UNIQUE NOT NULL)""")
    cursor.execute("""CREATE TABLE IF NOT EXISTS countries (id INTEGER PRIMARY KEY, name TEXT UNIQUE NOT NULL)""")
    cursor.execute("""CREATE TABLE IF NOT EXISTS genres (id INTEGER PRIMARY KEY, name TEXT UNIQUE NOT NULL)""")
    cursor.execute("""CREATE TABLE IF NOT EXISTS directors (id INTEGER PRIMARY KEY, name TEXT UNIQUE NOT NULL)""")
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS movies (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            title TEXT NOT NULL,
...
def load_movies():
    ...
    for row in cursor.fetchall():
        movie_id = row[0]
        values = row[1:]
        tree.insert("", tk.END, iid=movie_id, values=values)
    conn.close()

def open_add_ref(table_name):
    def save_ref():
    ...
    top = Toplevel(root)
    top.title(f"Lisa {table_name[:-1].capitalize()}")
    tk.Label(top, text=f"{table_name[:-1].capitalize()}:").pack(pady=5)
    entry = tk.Entry(top)
    entry.pack(pady=5)
    tk.Button(top, text="Salvesta", command=save_ref).pack(pady=10)

def get_name_list(table):
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute(f"SELECT name FROM {table} ORDER BY name")
...
def edit_movie():
    ...
    if not movie:
        messagebox.showerror("Viga", "Filmi andmeid ei leitud")
        return

    title, director_id, year, genre_id, duration, rating, language_id, country_id, description = movie
    ...
    def update_movie():
        ...
        new_director_id = get_id_by_name("directors", director_var.get())
        new_genre_id = get_id_by_name("genres", genre_var.get())
        new_language_id = get_id_by_name("languages", language_var.get())
        new_country_id = get_id_by_name("countries", country_var.get())
        ...
        cursor.execute("""
            ...
            WHERE id = ?
        """, (new_title, new_director_id, new_year, new_genre_id, new_duration, new_rating, new_language_id, new_country_id, new_description, movie_id))
        conn.commit()
        conn.close()
        top.destroy()
        load_movies()
    ...
    # UI
    top = Toplevel(root)
    top.title("Muuda film")

    tk.Label(top, text="Pealkiri").grid(row=0, column=0)
    title_entry = tk.Entry(top, width=50)
    title_entry.insert(0, title)
    title_entry.grid(row=0, column=1)

    tk.Label(top, text="Režissöör").grid(row=1, column=0)
    director_var = StringVar()
    director_combo = ttk.Combobox(top, textvariable=director_var, values=get_name_list("directors"))
    director_combo.grid(row=1, column=1)
    cursor.execute("SELECT name FROM directors WHERE id=?", (director_id,))
    dname = cursor.fetchone()
    if dname: director_var.set(dname[0])

    tk.Label(top, text="Aasta").grid(row=2, column=0)
    year_entry = tk.Entry(top, width=50)
    year_entry.insert(0, year if year else "")
    year_entry.grid(row=2, column=1)

    tk.Label(top, text="Žanr").grid(row=3, column=0)
    genre_var = StringVar()
    genre_combo = ttk.Combobox(top, textvariable=genre_var, values=get_name_list("genres"))
    genre_combo.grid(row=3, column=1)
    cursor.execute("SELECT name FROM genres WHERE id=?", (genre_id,))
    gname = cursor.fetchone()
    if gname: genre_var.set(gname[0])

    tk.Label(top, text="Kestus").grid(row=4, column=0)
    duration_entry = tk.Entry(top, width=50)
    duration_entry.insert(0, duration if duration else "")
    duration_entry.grid(row=4, column=1)

    tk.Label(top, text="Reiting").grid(row=5, column=0)
    rating_entry = tk.Entry(top, width=50)
    rating_entry.insert(0, rating if rating else "")
    rating_entry.grid(row=5, column=1)

    tk.Label(top, text="Keel").grid(row=6, column=0)
    language_var = StringVar()
    language_combo = ttk.Combobox(top, textvariable=language_var, values=get_name_list("languages"))
    language_combo.grid(row=6, column=1)
    cursor.execute("SELECT name FROM languages WHERE id=?", (language_id,))
    lname = cursor.fetchone()
    if lname: language_var.set(lname[0])

    tk.Label(top, text="Riik").grid(row=7, column=0)
    country_var = StringVar()
    country_combo = ttk.Combobox(top, textvariable=country_var, values=get_name_list("countries"))
    country_combo.grid(row=7, column=1)
    cursor.execute("SELECT name FROM countries WHERE id=?", (country_id,))
    cname = cursor.fetchone()
    if cname: country_var.set(cname[0])

    tk.Label(top, text="Kirjeldus").grid(row=8, column=0)
    desc_entry = tk.Entry(top, width=50)
    desc_entry.insert(0, description if description else "")
    desc_entry.grid(row=8, column=1)

    tk.Button(top, text="Salvesta muudatused", command=update_movie).grid(row=9, column=0, columnspan=2, pady=10)
...
def open_add_movie():
    def save_movie():
        ...
        director_id = get_id_by_name("directors", director_var.get())
        genre_id = get_id_by_name("genres", genre_var.get())
        language_id = get_id_by_name("languages", language_var.get())
        country_id = get_id_by_name("countries", country_var.get())
    ...
    tk.Button(top, text="Salvesta", command=save_movie).grid(row=9, column=0, columnspan=2, pady=10)

def manage_refs(table_name):
    def load_refs():
        for row in tree.get_children():
            tree.delete(row)
        conn = get_conn()
    ...
    def save_changes():
        selected = tree.selection()
        if not selected:
            messagebox.showwarning("Hoiatus", "Vali üks kirje")
            return
        ref_id = selected[0]
        ...
        cursor = conn.cursor()
        try:
            cursor.execute(f"UPDATE {table_name} SET name=? WHERE id=?", (new_name, ref_id))
            conn.commit()
            load_refs()
        except sqlite3.IntegrityError:
            messagebox.showerror("Viga", "Selline nimi on juba olemas")
    ...
    def delete_ref():
        ...
        if not selected:
            ...
            return
        ref_id = selected[0]
        if messagebox.askyesno("Kinnita", "Kustutada see kirje?"):
            conn = get_conn()
            cursor = conn.cursor()
            # Check if this id is used in movies, block if so
            cursor.execute(f"""
                SELECT COUNT(*) FROM movies WHERE
                {'director_id' if table_name == 'directors' else
                 'genre_id' if table_name == 'genres' else
                 'language_id' if table_name == 'languages' else
                 'country_id'} = ?
            """, (ref_id,))
            count = cursor.fetchone()[0]
            if count > 0:
                messagebox.showerror("Viga", "Seda kirjet kasutatakse filmides ja seda ei saa kustutada")
                conn.close()
                return
            cursor.execute(f"DELETE FROM {table_name} WHERE id=?", (ref_id,))
            conn.commit()
            conn.close()
            load_refs()
            entry.delete(0, tk.END)
    ...
    def on_select(event):
        ...
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute(f"SELECT name FROM {table_name} WHERE id=?", (ref_id,))
        row = cursor.fetchone()
        conn.close()
        if row:
            entry.delete(0, tk.END)
    ...
    top = Toplevel(root)
    top.title(f"Halda {table_name}")

    tree = ttk.Treeview(top, columns=("name",), show="headings", height=10)
    tree.heading("name", text=f"{table_name[:-1].capitalize()} nimi")
    tree.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)
    tree.bind("<<TreeviewSelect>>", on_select)

    tk.Label(top, text=f"{table_name[:-1].capitalize()} nimi:").pack(padx=10)
    ...
    tk.Button(btn_frame, text="Salvesta muudatus", command=save_changes).pack(side=tk.LEFT, padx=5)
    tk.Button(btn_frame, text="Kustuta", command=delete_ref).pack(side=tk.LEFT, padx=5)
...
def delete_movie():
    ...
    if not selected:
        ...
        return
    movie_id = selected[0]
    if messagebox.askyesno("Kinnita", "Kustutada see film?"):
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM movies WHERE id=?", (movie_id,))
        conn.commit()
...
tk.Button(btn_frame, text="Lisa film", command=open_add_movie).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Muuda film", command=edit_movie).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Kustuta film", command=delete_movie).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Lisa režissöör", command=lambda: open_add_ref("directors")).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Lisa žanr", command=lambda: open_add_ref("genres")).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Lisa keel", command=lambda: open_add_ref("languages")).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Lisa riik", command=lambda: open_add_ref("countries")).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Halda režissööre", command=lambda: manage_refs("directors")).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Halda žanre", command=lambda: manage_refs("genres")).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Halda keeli", command=lambda: manage_refs("languages")).pack(side=tk.LEFT, padx=5)
tk.Button(btn_frame, text="Halda riike", command=lambda: manage_refs("countries")).pack(side=tk.LEFT, padx=5)
